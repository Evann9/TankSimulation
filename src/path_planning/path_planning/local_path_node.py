...
현재 fusion 우선순위:
1. Projection-based YOLO-LiDAR cluster matching
   YOLO bbox 안으로 projection되는 DBSCAN cluster centroid를 객체 위치로 사용
2. Projection-based YOLO-LiDAR point matching
   YOLO bbox 안으로 projection되는 raw LiDAR point들의 median 위치 사용
3. Angle-based fallback
   기존 bbox 중심각 + LiDAR bearing gate 방식
"""
...
class LocalPathNode(Node):
    def __init__(self) -> None:
        ...
        # Fusion method and fallback settings.
        ...
        # Projection-based fusion settings. These share the same calibrated values as overlay.
        ...
        self.get_logger().info(
            "local_path_node started: projection fusion enabled="
    ...
    def timer_cb(self) -> None:
        fused = self.compute_fused_objects()
        self.fused_current = fused
        if fused:
            self.update_discovered_map(fused)
        self.publish_fused(fused)
        self.publish_current_markers(fused)
        self.publish_discovered()

    def compute_fused_objects(self) -> List[Dict[str, Any]]:
        if self.player_pose is None or self.latest_detections_payload is None:
            return []
    ...
    def _fuse_from_projected_clusters(
        ...
        return self._make_fused_object(
            ...
            matched_lidar_points=int(best["count"]),
            used_lidar_points=int(best["count"]),
            extra={"cluster_id": best["id"], "projection_uv": {"u": best["u"], "v": best["v"]}, "cluster_bbox": best.get("bbox")},
        )
    ...
    def _fuse_from_projected_points(
        ...
        return self._make_fused_object(
            ...
            source="yolo_lidar_projection_point_fusion",
            matched_lidar_points=len(candidates),
            used_lidar_points=len(selected),
    ...
    def _fuse_from_angle(
        ...
        return self._make_fused_object(
            class_name, confidence, bbox, est_x, est_y, est_z, px, py, pz, camera_heading,
            source="yolo_lidar_angle_fusion_fallback",
            matched_lidar_points=len(candidates),
            used_lidar_points=len(selected),
    ...
    def _make_fused_object(
        ...
        camera_heading: float,
        source: str,
        matched_lidar_points: int,
        used_lidar_points: int,
        ...
        obj = {
            ...
            "matched_lidar_points": matched_lidar_points,
            "used_lidar_points": used_lidar_points,
            "known_static": known_static is not None,
            "matched_static": known_static,
            "source": source,
        }
        if extra:
            obj.update(extra)
    ...
    def update_discovered_map(self, fused: List[Dict[str, Any]]) -> None:
    ...
    def publish_discovered(self) -> None:
        ...
        payload = {
            ...
            "candidate_count": len(self.discovered) - confirmed_count,
            "objects": [asdict(o) for o in self.discovered],
            "save_service": SERVICE_DISCOVERED_SAVE,
            "save_confirmed_only": self.save_confirmed_only,
        }
        msg = String()
        msg.data = json.dumps(payload, ensure_ascii=False)
        self.discovered_pub.publish(msg)
        self.discovered_marker_pub.publish(self.make_discovered_markers())
    ...
    def save_discovered_map(self) -> List[Path]:
        self.save_directory.mkdir(parents=True, exist_ok=True)
        payload = self.make_map_payload()
    ...
    def make_map_payload(self) -> Dict[str, Any]:
        obstacles = []
    ...
    def make_discovered_markers(self) -> MarkerArray:
        markers = MarkerArray()
        for idx, obj in enumerate(self.discovered):
    ...
    def _extract_lidar_point(self, point: Dict[str, Any], px: float, py: float) -> Optional[Dict[str, Any]]:
        ...
        if distance < self.min_fusion_range_m or distance > self.max_fusion_range_m:
            return None
        global_bearing = math.degrees(math.atan2(dx, dy))  # 0deg = +map.y
